...
public function showStats(ProduitRepository $produitRepository): Response
{
    // Fetch all products and their 'fav' counts
    $produits = $produitRepository->findAll();
    ...
    foreach ($produits as $produit) {
        $labels[] = $produit->getNomProd();
        $data[] = $produit->getFav() ?? 0; // Default to 0 if 'fav' is null
    }

    return $this->render('admin_prod/stats.html.twig', [
        'labels' => $labels,
        'data' => $data,
    ]);
}
